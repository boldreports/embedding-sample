...
function Create() {
    var dlgContent = "<div><label id = 'name-label'style = 'font-size:medium'>Name</label><input type='text' id='dialog-dashboard-name' name=dashboardname' placeholder='Enter the dashboard name' style='margin-left: 50px;width: 225px; font-size:medium '><br><br></div>";
    ...
    var dialogObj2 = new ejs.popups.Dialog({
        ...
        buttons: [
            {
                'click': () => {
                    var dbrdName = $("#dialog-dashboard-name").val();
                    dialogObj2.hide();
...
function CreateNew() {
    $(".nav-item #create-new").css("display", "none");
    $("#dbrd-name").css("display", "flex");
}

function CloseItem() {
    $(".nav-item-heading #create-new").css("display", "block");
    $("#dbrd-name").css("display", "none");
    $("#dashboard-name").val("");
}
